...
def test_full_workflow():
    ...
    # 11. Тест добавления второго перевала
    print("\n11. Тестирование добавления второго перевала...")
    second_test_data = {
        ...
        "photos": [
            {
                "url": "https://example.com/elbrus_photo.jpg"
            }
        ]
    }

    response = client.post("/submitData", json=second_test_data)
    assert response.status_code == 200
    second_pass_id = response.json()['id']
    print(f"   ✅ Второй перевал добавлен с ID: {second_pass_id}")

    # 12. Проверяем, что у пользователя теперь 2 перевала
    response = client.get("/submitData/?user__email=ivan.petrov@example.com")
    assert response.status_code == 200
    user_passes = response.json()
    print(f"   ✅ У пользователя теперь перевалов: {len(user_passes)}")

    # 13. Тест обновления несуществующего перевала
    print("\n13. Тестирование обновления несуществующего перевала...")
    response = client.patch("/submitData/888888", json=update_data)
